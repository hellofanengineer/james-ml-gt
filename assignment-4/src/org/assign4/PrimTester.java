...
public class PrimTester {
	...
	public PrimTester(int numLocations, double rewardMultiplier, int negRewardMod, double defaultCost) {
...
//		gridWorld = new PrimGridWorldDomain(22, 22);
		...
		int count = 1;
		for (Point p : gridWorld.getEndPoints()) {
			p.reward = rewardMultiplier * count++;
			// p.reward = rewardMultiplier * (double) (p.x + p.y);
			if (count % negRewardMod == 0) {
				p.reward *= -1.;
			}
			System.out.println("set reward = " + p.reward + " for " + p);
		}
		rf = new PointRewarder(gridWorld.getEndPoints(), defaultCost);
		tf = new SinglePFTF(domain.getPropFunction(GridWorldDomain.PFATLOCATION));
		goalCondition = new TFGoalCondition(tf);

		initialState = PrimGridWorldDomain.getOneAgentNLocationState(domain, numLocations);
		PrimGridWorldDomain.setAgent(initialState, gridWorld.getAgentStartPoint().x, gridWorld.getAgentStartPoint().y);
